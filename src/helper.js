...
export default Vue.mixin({
  ...
  computed: {
    ...
    // activeCurrency() {
    //   return this.siteSettings ? this.siteSettings.active_currency : null;
    // },
    activeCurrency_() {
      this.$store.dispatch('settings').then((response) => {
        if (response.data.error) {
        } else {
          this.activeCurrency__ = response.data.data.active_currency
        }
      }).catch((error) => {
        console.log('error server', error)
      })
      return this.activeCurrency__
    },
    setting(){
      this.$store.dispatch('settings').then((response) => {
        if (response.data.error) {
        } else {
          this.settings__ = response.data.data.settings
        }
      }).catch((error) => {
        console.log('error server', error)
      })
      return this.settings__
    },
    settings() {
  ...
  methods: {
    ...
    getUrl(url) {
      return 'https://yoori.spagreen.net/' + url
    },
    logout() {
    ...
    priceFormat(amount, only_amount) {
      ...
      if (this.activeCurrency_) {

        amount = amount * this.activeCurrency_.exchange_rate;
        ...
        if (this.setting) {
          let no_of_decimals, decimal_separator, thousands_separator, currency_symbol_format, fixed_amount, formatted_amount = '';
          no_of_decimals = this.setting.no_of_decimals;

          decimal_separator = this.setting.decimal_separator ? this.setting.decimal_separator : '.';
          thousands_separator = decimal_separator == ',' ? '.' : ',';
          currency_symbol_format = this.setting.currency_symbol_format ? this.setting.currency_symbol_format : 'amount_symbol';


          if (currency_symbol_format == 'amount_symbol')
            formatted_amount = this.$options.filters.currency(amount, this.activeCurrency__.symbol, no_of_decimals, {
              symbolOnLeft: false,
